...
Release dependencies: Resend sign-in/domain verification and Supabase SMTP configuration, genuine public Stripe webhook delivery verification in the intended environment, target database migrations/advisors, operator identity configuration, and deployment smoke checks. The configured Stripe test account had no webhook destinations at inspection. None of these live configuration steps is claimed complete. See `connected-workflow-release.md` for exact activation steps and test limitations.
