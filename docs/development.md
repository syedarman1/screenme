...
- `OPENAI_API_KEY`; optional `RESUME_AI_MODEL` defaults to `gpt-5.6-terra`.
...
`/privacy` and `/terms` describe the implemented service and providers. Operating business identity, retention practice and jurisdiction-specific policy decisions remain the operator's responsibility.
