...
import {
  validateCheckout,
  subscriptionIsPro,
  matchesOnsiteCheckout,
  publishableKey,
  subscriptionCancellationScheduled,
} from "../src/app/lib/billing";
...
test("checkout requires the paid product and verified owner", () => {
  ...
  assert.throws(() =>
    validateCheckout({ ...session, payment_status: "unpaid" }, "owner"),
  );
  assert.throws(() =>
    validateCheckout({ ...session, line_items: undefined }, "owner"),
  );
});
...
test("an old paid session does not confer an active subscription", () => {
  const sub = {
    status: "active",
    items: { data: [{ price: { id: "price_expected" } }] },
...
test("on-site checkout reuse isolates customer, price, surface, and return origin", () => {
  const onsite = {
    ...session,
    metadata: { userId: "owner", priceId: "price_expected" },
    ui_mode: "elements",
    return_url:
      "https://www.screenme.dev/success?session_id={CHECKOUT_SESSION_ID}",
...
test("browser payment keys must match the server's Stripe mode", () => {
  ...
  try {
    ...
    if (publicKey === undefined) delete process.env.STRIPE_PUBLISHABLE_KEY;
    else process.env.STRIPE_PUBLISHABLE_KEY = publicKey;
  }
});

test("portal cancellations recognize explicit cancel_at as well as the period-end flag", () => {
  assert.equal(
    subscriptionCancellationScheduled({
      status: "active",
      cancel_at_period_end: false,
      cancel_at: 1791606867,
    }),
    true,
  );
  assert.equal(
