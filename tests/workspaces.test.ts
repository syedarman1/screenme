...
before(async () => {
  process.env.NEXT_PUBLIC_SUPABASE_URL = "https://workspace.supabase.invalid";
...
const req = (method: string, body?: unknown, query = "") =>
  new Request(`http://localhost/api/workspaces${query}`, {
    method,
    headers: {
...
test("workspace inputs are bounded and reject malformed report structures", () => {
  ...
  assert.equal(
    workspacePayload.safeParse({
      ...blankWorkspace(),
      result: { version: "2.0", assessments: {} },
    }).success,
    false,
  );
});
test("workspace endpoints require authenticated ownership and never use caller user IDs", async () => {
  ...
  const r = await route.POST(
    req("POST", {
      ...
      revision: 0,
      user_id: "attacker",
      payload: blankWorkspace(),
    }),
  );
  assert.equal(r.status, 201);
  assert.equal(calls.at(-1)?.body.user_id, uid);
...
test("updates and deletes scope to owner and revision; stale writes get 409", async () => {
  ...
  assert.equal(
    (await route.DELETE(req("DELETE", { id, revision: 2 }))).status,
    200,
  );
  assert.match(calls.at(-1)!.url, /user_id=eq/);
});
test("history reads are scoped to the verified owner", async () => {
  assert.equal(
    (await route.GET(req("GET", undefined, `?id=${id}&versions=1`))).status,
    200,
  );
  const url = new URL(calls.at(-1)!.url);
  assert.equal(url.searchParams.get("user_id"), `eq.${uid}`);
  assert.equal(url.searchParams.get("workspace_id"), `eq.${id}`);
});
test("rewrites cannot invent numerical results even when a job requests them", () => {
  assert.doesNotThrow(() =>
    rejectNewNumbers(
      "Improved latency from 800ms to 240ms.",
      "Cut latency from 800ms to 240ms.",
    ),
  );
  assert.throws(() =>
    rejectNewNumbers("Supported customer accounts.", "Grew accounts by 40%."),
  );
});
