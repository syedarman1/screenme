...
export const writingInput = z.object({
  resume: z.string().trim().max(50000).default(""),
  job: z.string().trim().max(25000).default(""),
  company: z.string().max(200).default(""),
  jobTitle: z.string().max(200).default(""),
  tone: z
    .enum(["Professional", "Enthusiastic", "Concise", "Formal", "Creative"])
    .default("Professional"),
  passage: z.string().max(2000).default(""),
...
export function rejectNewNumbers(source: string, output: string) {
  ...
  for (const number of numbers(output))
    if (!allowed.has(number))
      throw new AnalysisValidationError(
        "The draft added an unsupported number.",
      );
}
const purposes: Record<WritingKind, string> = {
  tailor:
    "Produce a tailored resume using ONLY the candidate's documented facts. Preserve names, credentials, dates and numerical values exactly. Reorder and clarify relevant material; do not convert job requirements into candidate claims. Do not silently remove career history. Return the full resume as content and questions as an empty array.",
  letter:
    "Produce a ready-to-edit cover letter of 180–300 words, using the requested tone. Candidate claims must come only from the resume. Company claims must come only from the supplied posting or company field. No invented motivation, childhood stories, credentials, achievements, or inferred numbers. Use Dear Hiring Manager and Sincerely; no placeholder claims. Return the letter as content and questions as an empty array.",
  interview:
    "Produce six job-specific interview questions, one of each question type. Model answers are ANSWER OUTLINES: tell the user how to construct an honest answer; never write an invented personal success story. Reference supplied experience only when present. Hypothetical situational responses must be clearly conditional. Include practical coaching tips. Return questions and a short content introduction explaining that outlines must be personalized truthfully.",
  improve:
...
export async function generateWriting(kind: WritingKind, input: WritingInput) {
  if (kind !== "interview" && input.resume.length < 100)
    throw new AnalysisInputError(
      "Include at least 100 characters of resume text.",
    );
  if (["tailor", "interview"].includes(kind) && input.job.length < 50)
    throw new AnalysisInputError(
      "Include the job requirements and responsibilities.",
    );
  if (kind === "letter" && (!input.company.trim() || !input.jobTitle.trim()))
    throw new AnalysisInputError("Include the company and job title.");
  if (
    kind === "improve" &&
    (!input.passage.trim() || !input.resume.includes(input.passage))
  )
    throw new AnalysisInputError(
      "Choose an unchanged passage from the resume.",
    );
  ...
  const settings = /^gpt-[56]/.test(model)
    ? { reasoning_effort: "low" as const, max_completion_tokens: 6500 }
      ...
      messages: [
        ...
        },
        {
          ...
          content: JSON.stringify({
            kind,
            ...input,
            resumePassages: sourcePassages(input.resume, "R"),
          }),
        },
      ],
      response_format: zodResponseFormat(outputSchema, "career_writing_v2"),
    },
    { signal },
  );
  ...
  if (!draft.usable)
    throw new AnalysisInputError(
      "Use a resume and a real job description for this tool.",
    );
  if (
    !draft.content.trim() ||
    draft.content.length > 50000 ||
    JSON.stringify(draft).length > 80000
  )
    throw new AnalysisValidationError("Invalid draft length.");
  if (kind === "interview") {
    if (
    ...
    rejectNewNumbers(
      input.resume + (kind === "improve" ? input.facts : ""),
      draft.content,
    );
    if (kind === "improve" && draft.content.length > 2500)
      throw new AnalysisValidationError("The edit is too long.");
  }
  // A separate check evaluates claims against the original source, not the writer's own citations.
  const verify = await client.beta.chat.completions.parse(
    {
      model,
      ...settings,
      messages: [
  ...
  recordCompletion(verify);
  if (verify.choices[0]?.message.parsed?.supported !== true)
    throw new AnalysisValidationError(
      "The draft could not be verified against the supplied facts.",
    );
  return artifactSchema.parse({
    version: "2.0" as const,
    content: draft.content,
    ...(kind === "interview" ? { questions: draft.questions } : {}),
