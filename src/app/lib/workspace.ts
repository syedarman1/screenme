...
export const workspaceKind = z.enum([
  "scan",
  "match",
  "tailor",
  "letter",
...
export const workspacePayload = z.object({
  resume: z.string().max(50000),
  job: z.string().max(25000),
  targetRole: z.string().max(120),
  company: z.string().max(200).default(""),
  jobTitle: z.string().max(200).default(""),
  tone: z.string().max(30).default("Professional"),
  result: savedResultSchema.nullable(),
  reportResume: z.string().max(50000).default(""),
  reportJob: z.string().max(25000).default(""),
  notes: z.string().max(10000).default(""),
});
export type WorkspacePayload = z.infer<typeof workspacePayload>;
export const blankWorkspace = (): WorkspacePayload => ({
  resume: "",
  job: "",
  targetRole: "",
  company: "",
...
export type WorkspaceSummary = Pick<
  WorkspaceRow,
  "id" | "kind" | "title" | "revision" | "updated_at"
>;
...
export const WORKSPACE_PATHS: Record<WorkspaceKind, string> = {
  scan: "/resume",
  match: "/jobmatch",
  tailor: "/tailor",
  letter: "/coverLetter",
