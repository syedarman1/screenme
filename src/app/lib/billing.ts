...
export function appUrl(): string {
  ...
  if (process.env.NODE_ENV === "production" && url.protocol !== "https:")
    throw new Error("A secure application URL is required.");
  return url.origin;
}
...
export function matchesOnsiteCheckout(
  ...
  return (
    ...
    session.return_url === `${origin}/success?session_id={CHECKOUT_SESSION_ID}`
  );
}
...
export function subscriptionCancellationScheduled(
  sub: Pick<
    Stripe.Subscription,
    "cancel_at_period_end" | "cancel_at" | "status"
  >,
): boolean {
  // The customer portal can set an explicit cancel_at without setting the period-end flag.
  return (
    sub.status !== "canceled" &&
    (sub.cancel_at_period_end || Boolean(sub.cancel_at))
  );
}
...
export function validateCheckout(
  ...
  if (!session.line_items?.data.some((item) => item.price?.id === proPriceId()))
    throw new Error("Unexpected checkout product.");
}
...
export async function syncSubscription(args: {
  ...
  const status =
    !pro && ["active", "trialing"].includes(sub.status)
      ? "canceled"
      : sub.status;
...
export async function fulfillCheckout(
  ...
  return syncSubscription({
    userId,
    subscriptionId,
    key: `verify:${session.id}:${crypto.randomUUID()}`,
  });
}
