import { recordCompletion, recordUnpricedCall } from "./aiMetrics";
...
export async function interviewAudio(req: Request) {
  ...
  try {
    ...
    if (!(audio instanceof File) || !audio.size || audio.size > 8 * 1024 * 1024)
      return response({ error: "Record a short answer under 8 MB." }, 400);
    if (
      !/^(audio\/(webm|mp4|mpeg|ogg|wav|x-wav)|video\/webm)(;.*)?$/.test(
        audio.type,
      )
    )
      return response(
    ...
    if (typeof raw !== "string" || raw.length > 60000)
      return response(
        {
          error: "Interview history is too long. Start a new practice session.",
        },
        400,
      );
    const parsed = historySchema.safeParse(JSON.parse(raw));
    if (!parsed.success)
      return response({ error: "Invalid interview history." }, 400);
    ...
    if (!reply)
      return response(
        {
          error:
            "No coaching response was returned. Your allowance has not been used.",
        },
        502,
      );
    ...
    return response(
      {
        error:
          error instanceof SyntaxError
            ? "Invalid interview history."
            : "Interview practice is temporarily unavailable. Your allowance has not been used.",
      },
