    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <Link
            href="/login"
            className="px-6 py-2.5 bg-accent text-accent-fg rounded-lg text-sm font-semibold"
          >
            Sign in
          </Link>
        </div>
      ...
      <div className="page-inner">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {atLimit && !isPro ? (
                <Link
                  href="/dashboard"
                  className="flex items-center gap-2 px-4 py-2.5 bg-orange hover:opacity-90 text-bg text-sm font-semibold rounded-lg transition-colors"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
        ...
        <div className="flex gap-3 mb-8 overflow-x-auto pb-1">
          {COLUMNS.map((col) => (
            <div
              key={col.key}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border bg-surface min-w-fit`}
            >
          ...
          <div
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border min-w-fit ${atLimit && !isPro ? "border-[#ff9f0a]/30 bg-orange/[0.06]" : "border-border bg-surface"}`}
          >
            <span className="text-xs font-medium text-fg-muted">Total</span>
            <span
              className={`text-sm font-semibold tabular-nums ${atLimit && !isPro ? "text-orange" : "text-fg"}`}
            >
              {apps.length}
              {!isPro && (
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <svg
              className="animate-spin h-7 w-7 text-fg"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
          ...
          <div className="relative">
              ...
              {COLUMNS.map((col) => {
                ...
                return (
                  <div key={col.key} className="min-w-[260px] flex-1">
                    {/* Column header */}
                    <div
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg ${col.bg} mb-3`}
                    >
                      <span className={`w-2 h-2 rounded-full ${col.dot}`} />
                      ...
                      <span
                        className={`text-xs font-medium ${col.color} opacity-60 ml-auto`}
                      >
                        {colApps.length}
                      </span>
                    </div>
            ...
            <div
              className="pointer-events-none absolute inset-y-0 -right-1 w-12 bg-gradient-to-l from-bg to-transparent"
              aria-hidden="true"
            />
          </div>
        ) : (
          ...
          <div className="bg-surface rounded-lg border border-border overflow-hidden">
            ...
            {apps.map((app) => {
              ...
              return (
                <div
                  key={app.id}
                  className="grid grid-cols-[1fr_1fr_120px_100px_80px] gap-4 px-5 py-3.5 border-b border-border last:border-0 hover:bg-bg/60 transition-colors items-center"
                >
                  <Link
                    href={`/applications/${app.id}`}
                    className="text-sm font-medium text-fg truncate underline"
                  >
                    {app.company}
                  </Link>
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 text-xs font-medium ${col.color}`}
                  >
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-bg transition-colors cursor-pointer"
                      title="Edit"
                    >
                      <svg
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-red-bg transition-colors cursor-pointer"
                      title="Delete"
                    >
                      <svg
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-40 bg-surface border border-border rounded-lg shadow-lg py-1 z-50">
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs text-fg-muted hover:bg-bg hover:text-fg transition-colors cursor-pointer"
                >
                  Edit details
                </button>
                <div className="my-1 mx-2 border-t border-border" />
                <p className="px-3 py-1 text-[10px] text-fg-subtle uppercase tracking-wider">
                  Move to
                </p>
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs text-red hover:bg-red-bg transition-colors cursor-pointer"
                >
                  Delete
                </button>
              </div>
      ...
      <div className="flex items-center gap-3 mt-3">
        ...
        {app.url && (
          <a
            ...
            className="text-[10px] text-fg hover:underline truncate max-w-[120px]"
          >
            Link
          </a>
        )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <form
        ...
        className="relative bg-surface rounded-lg border border-border shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-7"
      >
        <div className="flex items-center justify-between mb-6">
          <h2 id="app-modal-title" className="text-lg font-semibold text-fg">
        ...
        {err && (
          <div className="mb-4 p-3 rounded-lg pill-danger text-sm">{err}</div>
        )}
          ...
          <details className="group">
            <summary className="text-xs font-medium text-fg cursor-pointer hover:underline">
              {jobDesc
                ...
                : "Add job description (for cover letter / job match)"}
            </summary>
            <textarea
              value={jobDesc}
